...
/// Fournit le générateur à l'arbre de composants Leptos
pub fn provide_id_generator() {
    provide_context(IdGenerator::new());
}

/// Hook pour récupérer le générateur
pub fn use_id() -> impl Fn() -> String + 'static {
    let r#gen = use_context::<IdGenerator>()
        .expect("IdGenerator doit être fourni dans un provide_context supérieur.");
    
    move || r#gen.next_id_str()
}
