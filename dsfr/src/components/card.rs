...
pub fn Card(
    ...
    view! {
        <div class=format!("{class} flex flex-col border border-gray-300 dark:border-gray-700 overflow-hidden")>
            ...
            <div class="flex flex-col gap-2 p-4">
                <h3 class="text-lg font-bold text-gray-900 dark:text-gray-100">
                    {match href.clone() {
                        Some(href) => view! { <a href=href class="hover:text-blue-france dark:text-blue-france-925">{title}</a> }.into_any(),
                        None => view! { <span>{title}</span> }.into_any(),
                    }}
                </h3>
                {description.map(|description| view! { <p class="text-sm text-gray-700 dark:text-gray-300">{description}</p> })}
